...
/**
 * The BitOutputStream and BitInputStream classes provide the ability to write
 * and read individual bits to a file in a compact form. One minor limitation of
 * this approach is that the resulting file will always have a number of bits
 * that is a multiple of 8. In effect, whatever bits are output to the file are
 * padded at the end with 0s to make the total number of bits a multiple of 8.
 * 
...
public class BitOutputStream extends PrintStream {
     ...
     * For most characters, this just means enclosing it in ' ' marks. For special
     * characters such as \n, replaces them with a \ and an n. Also replaces the
     * special EOF character with the string "EOF". Most characters with low ASCII
     * values (< 32) are printed as a decimal number.
     * 
    ...
    /**
     * true if still open for writing.
     */
    ...
    /**
     * true if writing bits; false to debug ASCII.
     */
    private boolean bitMode; // true if writing bits; false to debug ASCII
    /**
     * true if this output stream has written EOF.
     */
     ...
     * 
     * @param output  the target output stream to write.
     * @param bitMode true to write bits at a time; false to write ASCII characters
     *                (bytes) at a time for debugging.
     * @throws BitIOException if the file cannot be opened for writing.
     */
    ...
    /**
     * Flushes the buffer. If numDigits < BYTE_SIZE, this will effectively pad the
     * output with extra 0s, so this should be called only when numDigits ==
     * BYTE_SIZE or when we are closing the output.
     */
    public void flush() {
        if (inBitMode()) {
            write(digits);
     ...
     * students should not call this method. This is a hack so that a BitInputStream
     * knows when it is out of bits.
     * 
    ...
    /**
     * Returns whether this stream is in real 'bit mode', writing a bit to the file
     * for each call to writeBit. The alternative is 'byte mode', where a full byte
     * (ASCII character) of '0' or '1' is written for each call to writeBit, to make
     * it easier to debug your program.
     * 
    ...
    /**
     * Sets whether this stream is in real 'bit mode', writing a bit to the file for
     * each 0 or 1 written in writeBit (as described under inBitMode). Ignores the
     * caller and always uses 'byte' mode if writing to System.out or if the JVM
     * bitstream.bitmode environment variable is set.
     * 
    ...
    /**
     * Sets this bit output stream to use the given pattern of bits as its encoding
     * for the end of a file (EOF); students should not call this method.
     * 
    ...
    public void write(int b) {
        if (b == EOF || seenEOF) {
            ...
            if (DEBUG)
                System.out.println("  ** BitOutputStream write: " + b + " (" + toPrintable((char) b) + ")");
            try {
                output.write(b);
            } catch (IOException e) {
                throw new BitIOException(e);
    ...
    public void writeBit(int bit) {
        ...
        if (inBitMode()) {
            ...
            numDigits++;
            if (DEBUG)
                System.out.println("  ** BitOutputStream writeBit: " + bit);
     ...
     * @throws IllegalArgumentException if the string contains any characters other
     *                                  than '0' or '1'.
     */
        ...
        for (int i = 0; i < bits.length(); i++) {
            ...
            if (ch == '0' || ch == 0) {
                ...
                throw new IllegalArgumentException(
                        "Illegal bit value '" + ch + "' at index " + i + " of string \"" + bits + "\"");
            }
        }
    }
    ...
    public void writeBytes(String bytes) {
        for (int i = 0; i < bytes.length(); i++) {
            ...
            if (ch == '0' || ch == 0) {
                ...
                throw new IllegalArgumentException(
                        "Illegal bit value '" + ch + "' at index " + i + " of string \"" + bytes + "\"");
            }
        }
    }

    /**
     * Runs when the object is garbage collected / program shuts down. Included to
     ...
     */
    /**
     * A class to represent bit I/O errors as runtime exceptions. Used to avoid the
     * forced checked exceptions usually thrown in Java I/O.
     */
