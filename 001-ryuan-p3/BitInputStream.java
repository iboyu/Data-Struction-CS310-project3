...
public class BitInputStream extends InputStream {
    ...
    /**
     * another stream to monitor for EOF.
     */
     ...
     *                              (The partner can be null; simply won't partner
     *                              with any bit output stream in such a case.)
     */
     ...
     * file for each call to readBit. The alternative is 'byte mode', where a full
     * byte (character) is read from the file each time readBit is called, to make
    ...
    public int read() {
        int result = readByte();
        if (BitOutputStream.DEBUG)
            System.out.println(
                    "  ** BitInputStream read: " + result + " (" + BitOutputStream.toPrintable((char) result) + ")");
        return result;
    }

    /**
     * Reads and returns some bytes of information from this stream into the given
     * array.
     * 
     ...
     * @throws BitIOException                 if the input stream cannot be read.
     * @throws NullPointerException           if bytes is null.
     * @throws ArrayIndexOutOfBoundsException if offset + length is past the end of
     *                                        the array.
     */
    @Override
    ...
    public int readBit() {
        ...
        if (hasNextBit()) {
            if (inBitMode()) {
                ...
                result = digits % 2;
                if (BitOutputStream.DEBUG)
                    System.out.println("  ** BitInputStream readBit: " + result);
                digits /= 2;
                numDigits++;
                ...
                if (result != '0' && result != '1') {
                    throw new BitOutputStream.BitIOException(
                            "Illegal bit (byte) value: " + result + " (this is probably a binary-compressed file)");
                }
                result -= '0';
            }
        }
    ...
    /**
     * Reads and returns an entire line of text from this bit input stream as a
     * String. You would not normally want to call this method while you're reading
     * bits from a bit input stream; it is provided as a convenience for students
     * doing extra credit functionality in the assignment.
     * 
    ...
    public String readLine() {
        ...
        if (BitOutputStream.DEBUG)
            System.out.println("  ** BitInputStream readLine: \"" + line + "\"");
        return line.toString();
    }

    /**
     * Sets whether this stream is in real 'bit mode', reading a bit from the file
     * for each call to readBit (as described under inBitMode). Ignores the caller
     * and always uses 'byte' mode if reading from System.in or if the JVM
    ...
    public void setBitMode(boolean bitMode) {
        this.bitMode = bitMode && input != System.in && System.getProperty("bitstream.bitmode") == null;
    }

    /**
     * Called by Java when the program is shutting down; included to help ensure
    ...
    /**
     * a hack so that the input stream can detect the outputting
     * of a pseudo-EOF character to a partnered output stream.
     * @param partner bit out put stream parter
     */
